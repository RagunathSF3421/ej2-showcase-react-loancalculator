import { ColumnDirective, ColumnsDirective, DetailRow, GridComponent, GridModel, Inject, Page } from '@syncfusion/ej2-react-grids';
...
export default function App() {
  ...
  function refreshComponents() {
    (loanNumRef.current as any).refresh();
    (loanSliderRef.current as any).refresh();
    (pie.current as any).refresh();
    (accChart.current as any).refresh();
    (grid.current as any).refresh();
  }
  ...
  function refreshUI(): void {
    ...
    (loanNumRef.current as any).format = loanNumFormat;
    (loanNumRef.current as any).min = loanNumMinValue;
    (loanNumRef.current as any).max = loanNumMaxValue;
    (loanNumRef.current as any).value = loanValue;
    (loanNumRef.current as any).step = loanNumStep;

    (loanSliderRef.current as any).min = loanMinValue;
    (loanSliderRef.current as any).max = loanMaxValue;
    (loanSliderRef.current as any).step = loanStep;
    (loanSliderRef.current as any).ticks = loanTicks;
    (loanSliderRef.current as any).value = loanValue;

    (accChart.current as any).dataSource = yearWiseData;
    (grid.current as any).dataSource = yearWiseData;
    child.dataSource = dataUnits;
    (pie.current as any).dataSource = [
      {
        'x': 'Principal Amount',
        y: principalValue
      },
    ...
    (princAmount.current as any).innerHTML = principalAmt;
    (totalInt.current as any).innerHTML = interestAmt;
    (totalPay.current as any).innerHTML = totalAmt;
    (monPayment.current as any).innerHTML = emiAmt;
  }
  ...
  function onclickEvent(args: any) {
    ...
    if (target.classList.contains('e-row-toggle') || target.parentElement.querySelector('.e-row-toggle')) {
      ...
      if (target.classList.contains('e-icon-gdownarrow')) {
        ...
        (grid.current as any).detailRowModule.collapse(parseInt((closest(target, 'tr') as HTMLElement).getAttribute('aria-rowindex')!, 10));
      } else {
        target.classList.remove('e-icon-grightarrow');
        target.classList.add('e-icon-gdownarrow');
        (grid.current as any).detailRowModule.expand(parseInt((closest(target, 'tr') as HTMLElement).getAttribute('aria-rowindex')!, 10));
      }
    }
  }
      ...
      <div className="container main-content" id="content">
        ...
        <div className="row top-space loan-content max-content">
          ...
          <div style={{ color: 'white' }}>
            ...
            >
              <ColumnsDirective>
                <ColumnDirective headerText='Year' template={template} minWidth='80px' textAlign='Center' />
                <ColumnDirective field='yearTotal' headerText='Payment' minWidth='80px' format={format} textAlign='Center' hideAtMedia={paymentHideAtMedia} />
                <ColumnDirective field='yearPrincipal' headerText='Principal Paid' minWidth='80px' format={format} textAlign='Center' />
                <ColumnDirective field='yearInterest' headerText='Interest Paid' minWidth='80px' format={format} textAlign='Center' />
                <ColumnDirective field='endingBalance' headerText='Balance' minWidth='80px' format={format} textAlign='Center' />
              </ColumnsDirective>
